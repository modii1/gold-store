...
async function updateReturnForm(formData: FormData) {
  "use server";
  await updateReturnRequestAction(formData);
}
    ...
    <div className="space-y-5">
      ...
      {!returns?.length ? <div className="rounded-2xl border border-amber-100 bg-white p-12 text-center text-stone-400">لا توجد طلبات استرجاع</div> : returns.map((request: any) => (
        <article key={request.id} className="rounded-2xl border border-amber-100 bg-white p-5">
          ...
          <form action={updateReturnForm} className="mt-4 flex flex-col sm:flex-row flex-wrap gap-2"><input type="hidden" name="id" value={request.id} /><select name="status" defaultValue={request.status} className="rounded-xl border border-sand px-3 py-2 text-sm"><option value="pending">قيد المراجعة</option><option value="approved">مقبول</option><option value="rejected">مرفوض</option><option value="received">تم الاستلام</option><option value="refunded">تم رد المبلغ</option></select><input name="admin_note" placeholder="ملاحظة الإدارة" className="input-lux w-full sm:max-w-sm" /><button className="rounded-xl bg-ink px-4 py-2 text-sm font-bold text-white">حفظ القرار</button></form>
        </article>
      ))}
    </div>
  );
