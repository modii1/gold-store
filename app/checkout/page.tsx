...
export default async function CheckoutPage() {
  ...
  const { data: savedAddresses } = customer
    ? await createAdminClient().from("addresses").select("id,label,city,region,address,national_address,building_number,latitude,longitude,maps_url,is_default").eq("customer_identifier", customer.phone).order("is_default", { ascending: false }).order("created_at", { ascending: false })
    : { data: [] };
  ...
  return (
    <>
      ...
      <main className="flex-1 mx-auto max-w-5xl px-4 md:px-6 py-8">
        ...
        <CheckoutForm settings={settings} shipping={checkout.shipping} payment={checkout.payment} customer={customer} savedAddresses={savedAddresses || []} defaultCity={(checkout as { defaultCity?: string }).defaultCity} />
      </main>
