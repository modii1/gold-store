            ...
            return (
              <li key={item.id} className={`flex flex-wrap items-center gap-3 py-3 ${item.is_read ? "opacity-70" : ""}`}>
                ...
                <div className="flex items-center gap-1.5">
                  {item.order_id && (
                    <Link href="/account#orders" className="flex items-center gap-1 rounded-lg border border-sand px-2.5 py-1.5 text-[11px] font-semibold text-stone-600 transition hover:bg-cream">
      ...
      <div className="rounded-3xl border border-sand bg-white p-4 md:p-6">
        ...
        {prefs && (
          <div className="mt-4 space-y-3">
            <div className="hidden grid-cols-[1fr_repeat(3,90px)] gap-2 border-b border-sand pb-2 text-[11px] font-bold text-stone-500 sm:grid">
              <span>التصنيف</span>
              {CHANNELS.map((c) => <span key={c.key} className="text-center">{c.label}</span>)}
            </div>
            {PREF_CATEGORIES.map((cat) => (
              <div key={cat.key} className="grid grid-cols-1 gap-2 rounded-2xl bg-cream/60 p-3 sm:grid-cols-[1fr_repeat(3,90px)] sm:items-center">
                <div>
                  <p className="text-sm font-bold text-ink">{cat.label}</p>
                  <p className="text-[11px] text-stone-500">{cat.desc}</p>
                </div>
                {CHANNELS.map((c) => (
                  <label key={c.key} className="flex items-center justify-between gap-2 text-xs font-semibold text-stone-600 sm:justify-center">
